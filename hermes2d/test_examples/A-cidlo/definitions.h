...
{
public:
    CustomWeakFormPoisson(ProblemDefinition definition);
};

class CustomWeakFormPermitivity : public Hermes::Hermes2D::WeakForm<double>
{
public:
    CustomWeakFormPermitivity(ProblemDefinition definition);
};

class MyVolumetricIntegralCalculator : public PostProcessing::VolumetricIntegralCalculator<double>
{
public:
    MyVolumetricIntegralCalculator(MeshFunctionSharedPtr<double> source_function, ProblemDefinition definition) :
        PostProcessing::VolumetricIntegralCalculator<double>(source_function, 1), definition(definition)
    {
        ...
        add_labels(definition.labels_air, definition.EPS_AIR);
        add_labels(definition.labels_kartit, definition.EPS_KARTIT);
        add_labels(definition.labels_full, definition.EPS_FULL);
        add_labels(definition.labels_empty, definition.EPS_EMPTY);
    }
