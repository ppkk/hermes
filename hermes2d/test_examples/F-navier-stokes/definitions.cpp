...
{
public:
    WeakFormNSNewton(bool Stokes, bool Transient, double Reynolds, double time_step, Solution<double>* x_vel_previous_time, Solution<double>* y_vel_previous_time) : WeakForm<double>(3), Stokes(Stokes),
    Reynolds(Reynolds), time_step(time_step), x_vel_previous_time(x_vel_previous_time), y_vel_previous_time(y_vel_previous_time) {
      BilinearFormSymVel* sym_form_0 = new BilinearFormSymVel(0, 0, Stokes, Transient, Reynolds, time_step);
      add_matrix_form(sym_form_0);
      BilinearFormSymVel* sym_form_1 = new BilinearFormSymVel(1, 1, Stokes, Transient, Reynolds, time_step);
      add_matrix_form(sym_form_1);
      ...
      BilinearFormUnSymPressureXVel* unsym_pressure_velx_form = new BilinearFormUnSymPressureXVel(2, 0);
      add_matrix_form(unsym_pressure_velx_form);

      BilinearFormUnSymPressureYVel* unsym_pressure_vely_form = new BilinearFormUnSymPressureYVel(2, 1);
      add_matrix_form(unsym_pressure_vely_form);

      VectorFormNS_0* F_0 = new VectorFormNS_0(0, Stokes, Transient, Reynolds, time_step);
      F_0->ext = Hermes::vector<MeshFunction<double>*>(x_vel_previous_time, y_vel_previous_time);
      add_vector_form(F_0);
      VectorFormNS_1* F_1 = new VectorFormNS_1(1, Stokes, Transient, Reynolds, time_step);
  ...
  {
  public:
      BilinearFormSymVel(int i, int j, bool Stokes, bool Transient, double Reynolds, double time_step) : MatrixFormVol<double>(i, j), Stokes(Stokes), Transient(Transient),
      Reynolds(Reynolds), time_step(time_step) {
        sym = HERMES_SYM;
    }
    ...
    MatrixFormVol<double>* clone()
    {
      return new BilinearFormSymVel(this->i, this->j, this->Stokes, this->Transient, this->Reynolds, this->time_step);
    }
  protected:
    // Members.
    bool Stokes, Transient;
    double Reynolds;
    double time_step;
  };
...
/// antisym parts, for testing only
  class BilinearFormUnSymPressureXVel : public MatrixFormVol<double>
  {
  public:
    BilinearFormUnSymPressureXVel(int i, int j) : MatrixFormVol<double>(i, j) {
      sym = HERMES_NONSYM;
    }

    double value(int n, double *wt, Func<double> *u_ext[], Func<double> *u, Func<double> *v, Geom<double> *e, ExtData<double> *ext) const{
      return int_dudx_v<double, double>(n, wt, u, v);
    }

    Ord ord(int n, double *wt, Func<Ord> *u_ext[], Func<Ord> *u, Func<Ord> *v, Geom<Ord> *e, ExtData<Ord> *ext) const {
      return int_dudx_v<Ord, Ord>(n, wt, u, v);
    }

    MatrixFormVol<double>* clone()
    {
      return new BilinearFormUnSymPressureXVel(this->i, this->j);
    }
  };


  class BilinearFormUnSymPressureYVel : public MatrixFormVol<double>
  {
  public:
    BilinearFormUnSymPressureYVel(int i, int j) : MatrixFormVol<double>(i, j) {
      sym = HERMES_NONSYM;
    }

    double value(int n, double *wt, Func<double> *u_ext[], Func<double> *u, Func<double> *v, Geom<double> *e, ExtData<double> *ext) const{
      return int_dudy_v<double, double>(n, wt, u, v);
    }

    Ord ord(int n, double *wt, Func<Ord> *u_ext[], Func<Ord> *u, Func<Ord> *v, Geom<Ord> *e, ExtData<Ord> *ext) const {
      return int_dudy_v<Ord, Ord>(n, wt, u, v);
    }

    MatrixFormVol<double>* clone()
    {
      return new BilinearFormUnSymPressureYVel(this->i, this->j);
    }
  };
  ...
  {
  public:
    VectorFormNS_0(int i, bool Stokes, bool Transient, double Reynolds, double time_step) : VectorFormVol<double>(i), Stokes(Stokes), Transient(Transient), Reynolds(Reynolds), time_step(time_step) {
    }
    ...
    VectorFormVol<double>* clone()
    {
      return new VectorFormNS_0(this->i, this->Stokes, this->Transient, this->Reynolds, this->time_step);
    }
  protected:
    // Members.
    bool Stokes, Transient;
  ...
  {
  public:
    VectorFormNS_1(int i, bool Stokes, bool Transient, double Reynolds, double time_step) : VectorFormVol<double>(i), Stokes(Stokes), Transient(Transient), Reynolds(Reynolds), time_step(time_step) {
    }
    ...
    VectorFormVol<double>* clone()
    {
      return new VectorFormNS_1(this->i, this->Stokes, this->Transient, this->Reynolds, this->time_step);
    }
  protected:
    // Members.
    bool Stokes, Transient;
